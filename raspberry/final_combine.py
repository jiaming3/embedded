...
#this is the final code that combine all other py files
from __future__ import division, print_function
from nxp_imu import IMU
import time
from bmp280 import bmp280_readdata,bmp280_convert,bmp280_checktemp
from humidity_sensor import hum,temp
...
########
#this function convert sensor data from FXAS21002 and FXOS8700 to roll,pitch and yaw. However,yaw is not accurate enough without use of kalman filter and
# other alogorithm such as sensor fusion. We use adjust parameter of motor instead in order to achieve angle fix
def ahrs():
    #print('')
    ...
    # if the flower pot fall large than 50 degree on pitch and roll axis, than it will be recognise as "fall"
    if r>50 or p>50 :
        fall = True
    else:
        fall =False
...
if __name__ == "__main__":
    ...
    max_num = 2  # record maximum number of backward, when it reach to maximum number, it won't go backward anymore

    temp_threshold = 50
    humid_threshold = 80
    pressure_threshold = 1016

    water_time = 2 # how many senconds we add water for each time
    each_hr_for_water = 8 # every n hour for adding water
    ...
    try:
     while True:
        # start mqtt and read data from sensor
        client.loop_start()
        flower = ahrs()
        ...
        # read phone data and perform specific motion base on data from sensor
        try:
